...
<body class="bg-gray-50 text-gray-800 antialiased">
    ...
    <div id="profileDropdown" class="hidden fixed top-16 right-4 sm:right-8 lg:right-24 bg-white rounded-lg shadow-xl border border-gray-200 w-64 z-50">
        ...
        <div class="border-t border-gray-200">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="flex items-center gap-3 px-4 py-3 hover:bg-red-50 transition w-full text-left">
    ...
    <main class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-screen">
        ...
        <div class="bg-white rounded-xl shadow-sm p-8 mb-6 text-center">
            ...
            <p class="text-gray-600 mb-1">Selesaikan pembayaran sebelum {{ \Carbon\Carbon::parse($order->created_at)->addHours(24)->format('d M Y, H:i') }} WIB</p>
        </div>
            ...
            <div class="border-t pt-4">
                ...
                <div class="space-y-3">
                    <div class="flex gap-3">
                        <div class="w-6 h-6 rounded-full bg-red-600 text-white flex items-center justify-center flex-shrink-0 text-sm font-semibold">1</div>
                        <p class="text-sm text-gray-700">Buka aplikasi Livin' by Mandiri atau kunjung ATM Mandiri terdekat</p>
                    </div>
                    <div class="flex gap-3">
                        <div class="w-6 h-6 rounded-full bg-red-600 text-white flex items-center justify-center flex-shrink-0 text-sm font-semibold">2</div>
                        <p class="text-sm text-gray-700">Pilih menu <span class="font-semibold">Bayar/Beli</span> → <span class="font-semibold">Virtual Account</span></p>
                    </div>
                    <div class="flex gap-3">
                        <div class="w-6 h-6 rounded-full bg-red-600 text-white flex items-center justify-center flex-shrink-0 text-sm font-semibold">3</div>
                        <p class="text-sm text-gray-700">Masukkan nomor Virtual Account: <span class="font-semibold font-mono">8851 2345 6789 0123</span></p>
                    </div>
                    <div class="flex gap-3">
                        <div class="w-6 h-6 rounded-full bg-red-600 text-white flex items-center justify-center flex-shrink-0 text-sm font-semibold">4</div>
                        <p class="text-sm text-gray-700">Konfirmasi pembayaran sebesar <span class="font-semibold text-red-600">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span></p>
                    </div>
                    <div class="flex gap-3">
                        <div class="w-6 h-6 rounded-full bg-red-600 text-white flex items-center justify-center flex-shrink-0 text-sm font-semibold">5</div>
                ...
                <div class="flex gap-4">
                    <div class="w-16 h-16 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                        @if($item->product_image)
                            <img src="{{ asset('image/' . $item->product_image) }}" alt="{{ $item->product_name }}" class="w-full h-full object-cover" />
                        @else
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm font-semibold text-red-600 mt-1">Rp {{ number_format($item->price, 0, ',', '.') }}</p>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <p class="text-sm text-blue-800">
                <span class="font-semibold">Penting:</span> Pembayaran akan diverifikasi otomatis setelah berhasil. Jika ada kendala, hubungan customer service kami.
            </p>
        </div>
    </main>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div>
                    ...
                    <p class="text-sm text-neutral-400">Platform jual beli Ulos terpercaya untuk melestarikan tradisi Batak</p>
                    <div class="flex gap-3 mt-4">
                        <a href="#" class="w-8 h-8 rounded-full bg-neutral-800 flex items-center justify-center hover:bg-neutral-700 transition">
    ...
    <script>
        ...
        const lastButton = document.querySelectorAll('header button')[document.querySelectorAll('header button').length - 1];
