        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <a href="{{ route('keranjang') }}"
                        class="relative flex items-center gap-2 text-red-600 hover:text-red-700 transition">
                        <div class="relative">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div id="mobile-menu" class="md:hidden hidden pb-4">
                <nav class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <a href="{{ route('keranjang') }}"
                        class="flex items-center gap-3 px-4 py-3 text-red-600 hover:bg-red-50 font-medium">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.3 6.8a1 1 0 00.9 1.2H19m-7 4a1 1 0 100-2 1 1 0 000 2zm7 0a1 1 0 100-2 1 1 0 000 2z" />
            ...
            <section class="lg:col-span-2 space-y-4">
                @forelse($items as $item)
                    <div class="bg-white border rounded-xl shadow-sm">
                            ...
                            <div class="flex-1">
                                <div class="flex items-start justify-between">
                                    <div>
                                        ...
                                        @if ($item->tag)
                                            <span
                                                class="text-xs text-gray-500 inline-block mt-0.5">{{ $item->tag }}</span>
                                        @endif
                                    </div>
                                    <form id="delete-form-{{ $item->id }}"
                                        action="{{ route('cart.delete', $item) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="deleteCartItem({{ $item->id }}, this)"
                                            class="text-gray-400 hover:text-red-600" title="Hapus">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                viewBox="0 0 20 20" fill="currentColor">
                                                <path fill-rule="evenodd"
                                                    d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 100 2h.293l.853 10.24A2 2 0 007.141 18h5.718a2 2 0 001.995-1.76L15.707 6H16a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zm-1 6a1 1 0 012 0v6a1 1 0 11-2 0V8zm5 0a1 1 0 10-2 0v6a1 1 0 102 0V8z"
                                                    clip-rule="evenodd" />
                                            </svg>
                                        </button>
                                    </form>
                                </div>
                                <div class="mt-2 text-red-600 font-semibold">Rp
                                ...
                                <div class="mt-3 flex items-center gap-3 text-sm">
                                    ...
                                    <div class="inline-flex items-center border rounded-md overflow-hidden">
                                        ...
                                        <form action="{{ route('cart.qty', $item) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button name="qty" value="{{ $item->quantity + 1 }}"
                                                class="px-3 py-1 text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors">+</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div
                            class="px-4 py-3 bg-gray-50 border-t text-xs text-gray-500 flex items-center justify-between">
                            <span>Subtotal:</span>
                    ...
                    <div class="bg-white border rounded-xl shadow-sm p-8 text-center">
                        <div class="w-16 h-16 mx-auto mb-4 bg-gray-100 rounded-full flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-gray-400" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.3 6.8a1 1 0 00.9 1.2H19m-7 4a1 1 0 100-2 1 1 0 000 2z" />
                            </svg>
                        </div>
                    ...
                    <div class="mt-3 space-y-2 text-sm">
                        ...
                        @else
                            <div class="flex items-center justify-between text-amber-600 text-xs">
                                <span>Belanja Rp {{ number_format(500000 - $subtotal, 0, ',', '.') }} lagi untuk gratis
                                    ongkir</span>
                            </div>
                        @endif
                        <hr class="my-2">
                        <div class="flex items-center justify-between font-semibold">
                            ...
                            <span class="text-red-600">Rp
                                {{ number_format($subtotal >= 500000 ? $subtotal : $total, 0, ',', '.') }}</span>
                        </div>
                    </div>

                    @if ($items->count() > 0)
                        <a href="{{ route('checkout') }}"
                            class="mt-4 w-full bg-red-600 text-white py-2.5 rounded-md hover:bg-red-700 transition-colors font-medium block text-center">
                            Lanjut ke Pembayaran
                        </a>
                    @else
                        <button disabled
                            class="mt-4 w-full bg-gray-300 text-gray-500 py-2.5 rounded-md cursor-not-allowed font-medium">
                            Keranjang Kosong
                        </button>
                    @endif
        ...
        <div class="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-10">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8 text-sm">
                <div>
                    ...
                    <p class="mt-4 text-neutral-400">Platform jual beli Ulos terpercaya untuk melestarikan tradisi
                        Batak.</p>
                </div>
                <div>
            ...
            <div class="mt-8 border-t border-neutral-800 pt-4 text-center text-neutral-500 text-sm">©
                {{ date('Y') }} UlosTa. Semua hak dilindungi.</div>
        </div>
    </footer>
        ...
        document.querySelectorAll('form').forEach(form => {
            ...
            form.addEventListener('submit', function() {
                ...
                if (button) {
                    ...
                    button.innerHTML = button.innerHTML.includes('Loading') ? button.innerHTML : button
                        .innerHTML + ' <span class="ml-2">...</span>';
                }
            });
        });
